...
public class Grid {
	...
	for (int i = 0; i < row; i++) // 3. Changes col 0,1 0,2 0,3 1,0
	{
		...
		{
		    position.setRight(grid[i][j]); // 6. ASSIGNS RIGHT to
						   // position Node
		    position = grid[i][j]; // 7. Moves Position to the next cell
		    if (j == col - 1) // 8. Only if position is at the end of
				      // the row grid
		    {
			position.setRight(grid[i][0]); // 9. Link last cell to
						       // beginning of the row
			if (i + 1 < row) // 10. Before moving pos. check if
					 // there is a grid under 1+1<3
			    position = grid[i + 1][0];
		    } // 10. Move position down a row
		}
	    }
	    position = head;
		...
		{
		    if (I + j != 0) { // 5. If grid[i][j] is empty
			position.setDown(grid[j][I]); // 6. ASSIGNS RIGHT to
						      // position Node
			position = grid[j][I]; // 7. Moves Position
			if (j == row - 1)// 2=2 // 8. Only if position is at the
					 // end of the row grid
			{
			    position.setDown(grid[0][I]); // 9. Link last cell
							  // to beginning of the
							  // row
			    if (I + 1 < col) // 10. Before moving pos. check if
					     // there is a grid u
				position = grid[0][I + 1];
			} // 10. Move position down a row
		    }
	...
	for (int i = 0; i < colListLenght; i++) // Col. Title generator +
						// counter
	{
