...
public class Value
{
	private double dVal;
	private String sVal;
	private String tag;   //indicates which one of these two attributes represents the real value
                           //Ctr of value
    public Value(){ //
		this.dVal=0;
		this.sVal="null";
	...
	public void input(String string){
		if(  string.startsWith("\""))// grabs the value of the first letter of the "input"
		{
			setsVal( string );
	...
	public Value sum(Value valIn)// it says it only takes one Value as as paramenter????
	{//
	...
	public Value subtract(Value valIn)// it says it only takes one Value as as paramenter????
	{ //
		Value answer = new Value();
		double answer1;
	...
	public Value multiply(Value valIn)// it says it only takes one Value as as paramenter????
	{ 
		Value answer = new Value();
		double answer1;
